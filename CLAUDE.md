...
- Cloudflare follows a matching `_redirects` rule **even when a static asset exists at that path** — the opposite of what the old comment in `public/_redirects` claimed. The `/ar|/en/projects/:zone[/:slug] → /index.csr 200` wildcards therefore shadowed all 52 prerendered zone/project pages (valid URLs served the client shell or the homepage, invalid ones returned 200 soft 404s). `public/_redirects` is now generated by `scripts/generate-redirects.js` from `content-manifest.json`, emitting an explicit rewrite per known page ahead of any wildcard. Never add a wildcard over a prerendered path. (2026-09-20)
