...
- Schema is one graph: `buildOrganizationSchema()` is the only company node (`@id` `…/#organization`) and `index.html` ships the only `WebSite` (`@id` `…/#website`). Page-level nodes reference the company by `@id`; never inline another copy. NAP comes from `BUSINESS_LOCATION`/`SOCIAL_LINKS` in `social.config.ts` — `telephone` is the phone number, not WhatsApp. (2026-09-20)
- Follow-up (SEO content, needs the business): `/ar/sadat-city-maps/` still has ~95 words of unique prose. The structural work is done (36 unique anchors, ItemList urls, breadcrumb, outbound links); the group-level paragraphs need real knowledge of the city. (2026-09-20)
...
- **Highest-value open perf item.** Admin-uploaded images are served as untouched originals from R2 via `functions/uploads/[[path]].ts`: the homepage alone references 81.6 MB across 48 files, individually 2.2–4.2 MB PNGs. Nothing resizes or re-encodes them. This stayed hidden while those sections were `@defer (on viewport)` — Lighthouse never scrolled, so the images never loaded. Needs resizing at upload time (the Workers runtime cannot run sharp, so either browser-side downscaling in the admin form or Cloudflare Image Resizing). (2026-09-21)
- `@defer (hydrate on viewport)` makes a section's content server-rendered. Use it for text and links; do NOT use it for sections full of uploaded photos, which puts every one of them in the initial HTML. gallery-preview and projects-mosaic are deliberately plain `@defer` for this reason. (2026-09-21)
- The hero is WebP, not AVIF, on purpose: AVIF saved 6 KB at the width a phone picks and cost far more CPU to decode on a low-end device, which is what LCP is bound by there. (2026-09-21)
